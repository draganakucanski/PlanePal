    ...
    public class FlightService : IFlightService
    {
        ...
        public async Task LoadFlights()
        {
            ...
            if (apiResponse.Success)
            {
                ...
                {
                    if (scheduledFlightDTO.Flight_status == FlightStatus.SCHEDULED)
                    {
                        Random rand = new();
                        int randDays = rand.Next(1, 31);
                        scheduledFlightDTO.Departure.Scheduled = scheduledFlightDTO.Departure.Scheduled.AddDays(randDays);
                        scheduledFlightDTO.Arrival.Scheduled = scheduledFlightDTO.Arrival.Scheduled.AddDays(randDays);
                        scheduledFlightDTO.Flight_date = scheduledFlightDTO.Flight_date.AddDays(randDays);
                    }

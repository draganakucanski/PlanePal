...
namespace PlanePal.BookingUnitTests
{
    public class CreateBookingTests
    {
        ...
        [Fact]
        public async Task BookFlight_DepartureDateAlreadyPassed_ReturnsError()
        {
            // Arange
            var user = new User
            {
            ...
            var scheduledFlight = new ScheduledFlight
            {
                ...
                Departure = new Departure { Scheduled = DateTime.Now.AddDays(-1) }
            };
            ...
            // Arrange
            Assert.Null(result.Data);
            Assert.Equal("Cannot book the flight because the departure date has already passed", result.Message);
            Assert.False(result.Success);
        }
        ...
        public async Task BookFlight_FlightStatusIsNotScheduled_ReturnsError()
        {
            ...
            var scheduledFlight = new ScheduledFlight();
            scheduledFlight.Id = 1;
            scheduledFlight.Flight_date = DateTime.Now.AddDays(10);
            scheduledFlight.Departure = new()
            {
                Scheduled = DateTime.Now.AddDays(10)
            };
        ...
        public async Task BookFlight_TicketQuantityExceeds5_ReturnsError(int ticketQuantity)
        {
            ...
            scheduledFlight.Departure.Scheduled = DateTime.Now.AddDays(10);
